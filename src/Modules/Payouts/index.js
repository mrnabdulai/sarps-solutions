...
function Payouts() {
    ...
    const fetchData = async () => {
        ...
        try {
            ...
            dispatch(doSetPayouts(response.data))
    ...
    return (
        <section className='pr-5'>
            <div className="items-center flex justify-between mb-5">
                <AlSectionHeader>Payouts</AlSectionHeader>
                <AlButton text="Add Payout" onClick={() => {
                    navigate('/app/payouts/add')
                }} />
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                        {doneFetchingAndHasData() ? (data.map((payout, index) => {
                            ...
                            return (<tr key={payout.id}>
                                <td className="w-full max-w-0 py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:w-auto sm:max-w-none sm:pl-6">
                                    ...
                                    <dl className="font-normal lg:hidden">
                                        ...
                                        <dd className="mt-1 truncate text-gray-700">{payout.employeeId}</dd>
                                        <dt className="sr-only sm:hidden">title</dt>
                                        <dd className="mt-1 truncate text-gray-500 sm:hidden">{payout.amount}</dd>
                                    </dl>
                                </td>
                                <td className="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell">{payout.employeeId}</td>
                                ...
                                <td className="hidden px-3 py-4 text-sm text-center text-gray-500 sm:table-cell">{format(Date.parse(payout.createdAt), 'MM/dd/yyyy')}</td>
                                <td className="py-4 text-right text-sm font-medium ">
                                    <div className='h-full w-full flex items-center justify-center gap-x-4' >
                                        <AlIconButton Icon={EyeIcon} colorClass="text-primary" onClick={() => {
                                            navigate("/app/payouts/" + payout.id)
                                        }} />
                            ...
                            <td colspan={9} className="text-center py-4">
                                ...
                                <p className="mt-1 text-sm text-gray-500">Get started by adding a payout</p>
                                <div className="mt-6">
