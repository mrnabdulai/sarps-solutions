...
import { doSetAgents } from './duck/action'
...
function Agents() {
    ...
    const fetchData = async () => {
        ...
        try {
            setFetching(true)
            setFetchError("")
            const response = await Axios.get("/api/user/getAgents")
            ...
            dispatch(doSetAgents(response.data))
            console.log(response.data)
        } catch (err) {
            console.log(err)
            if (err.response) setFetchError(err.response.data[Object.keys(err.response.data)[0]])
            else setFetchError("An error occurred while fetching agents")
        }
        finally {
            setFetching(false)
    ...
    return (
        <section className='pr-5'>
            <div className="items-center flex justify-between mb-5">
                <AlSectionHeader>Agents</AlSectionHeader>
                <AlButton text="Add New Agent" onClick={() => {
                    navigate('/app/agents/add')
                }} />
          ...
          <span className="sr-only">Edit</span>
                    ...
                    <tbody className="divide-y divide-gray-200 bg-white">
                        {doneFetchingAndHasData() ? (data.map((agent, index) => {
                            ...
                            return (<tr key={agent.id}>
                                <td className="w-full max-w-0 py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:w-auto sm:max-w-none sm:pl-6">
                                    ...
                                    <dl className="font-normal lg:hidden">
                                        ...
                                        <dd className="mt-1 truncate text-gray-700">{agent.firstName} {agent.lastName}</dd>
                                        <dt className="sr-only sm:hidden">email</dt>
                                        <dd className="mt-1 truncate text-gray-500 sm:hidden">{agent.email}</dd>
                                    </dl>
                                </td>
                                <td className="hidden px-3 py-4 text-sm text-gray-500 lg:table-cell">{agent.firstName} {agent.lastName}</td>
                                ...
                                <td className="hidden px-3 py-4 text-sm text-center text-gray-500 sm:table-cell">{format(Date.parse(agent.createdAt), 'MM/dd/yyyy')}</td>
                            ...
                            <td colspan={10} className="text-center py-4">
                                ...
                                <h3 className="mt-2 text-sm font-medium text-gray-900">No agents have been added yet</h3>
                                <p className="mt-1 text-sm text-gray-500">Get started by adding an agent</p>
                                <div className="mt-6">
